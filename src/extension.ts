...
// 1. DEFINE DEFAULTS HERE (Moved from package.json)
const DEFAULT_STYLES: KeywordConfig[] =  [
    // ===== Declarations =====
...
export function activate(context: vscode.ExtensionContext) {
    console.log('Keyword Colorizer: Active');
    ...
    vscode.workspace.onDidChangeConfiguration(e => {
        if (e.affectsConfiguration('keywordColorizer.styles')) {
            triggerUpdateDecorations();
        }
    }, null, context.subscriptions);
}
...
function triggerUpdateDecorations() {
    ...
    const config = vscode.workspace.getConfiguration('keywordColorizer');
    // Get user settings (defaults to empty array now)
    const userStyles: KeywordConfig[] = config.get('styles') || [];

    // MERGE STRATEGY: 
    // 1. Create a map of defaults
    const styleMap = new Map<string, KeywordConfig>();
    DEFAULT_STYLES.forEach(s => styleMap.set(s.keyword, s));

    // 2. Overwrite with user settings
    userStyles.forEach(s => styleMap.set(s.keyword, s));

    // 3. Convert back to array
    const finalStyles = Array.from(styleMap.values());
    const text = activeEditor.document.getText();

    finalStyles.forEach(style => {
        if (!style.keyword || !style.color) return;
        ...
        const escapedKeyword = style.keyword.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
